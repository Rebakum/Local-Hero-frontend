      ...
      <div className="container-lh py-8 sm:py-10 md:py-12">
        ...
        <div className="grid grid-cols-1 items-center gap-8 md:grid-cols-12">
          ...
          <div className="md:col-span-7">
            ...
            <div className="mt-6 space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-xs text-slate-500 font-semibold uppercase">Hourly Rate:</span>
                <span className="text-xl font-black text-slate-900">£{pro.hourlyRate || (pro as any).rate || '0'}/hr</span>
              </div>

              <div className="flex flex-wrap gap-3 sm:w-80">
                <button
                ...
                <button
                  ...
                  disabled={messaging}
                  className="flex items-center justify-center gap-2 rounded border-2 border-red-600 bg-white py-2.5 px-4 text-xs font-bold text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50"
                >
          ...
          <div className="flex justify-center md:col-span-5">
            <div className="relative flex h-60 w-72 items-center justify-center rounded-xl bg-slate-100 p-2 border border-slate-200 shadow-sm">
              <img
              ...
              <div className="absolute -left-3 top-4 flex h-20 w-20 rotate-[-12deg] items-center justify-center rounded-full bg-red-600 p-2 text-center text-white shadow-xl ring-4 ring-white">
                <div>
        ...
        {/* CORE SERVICES BOX */}
        <div className="mt-10 rounded-xl border border-slate-200 bg-slate-100/80 p-6 shadow-sm">
        ...
        {/* DETAILS & DISCLAIMERS */}
        <div className="mt-8 space-y-3 text-xs leading-relaxed text-slate-600">
          <p>
          ...
          <div className="grid grid-cols-1 items-end gap-6 pt-2 md:grid-cols-12">
            <div className="space-y-1.5 text-[11px] text-slate-500 md:col-span-9">
              ...
              <p className="font-bold text-slate-800 pt-1">NO EXCESS OR HIDDEN FEES & NO CALL OUT FEE</p>
            </div>

            {/* Gas Safe / Certified Badge */}
            <div className="flex justify-start md:col-span-3 md:justify-end">
              <div className="flex items-center gap-1.5 rounded-br-2xl bg-amber-400 p-2.5 font-black text-slate-900 shadow">
                <div className="rounded bg-slate-900 px-1.5 py-0.5 text-[10px] text-amber-400">GAS</div>
        ...
        <div className="mt-10 border-t border-slate-200 pt-6">
          <div className="flex gap-2 border-b border-slate-200 pb-2">
            <button
              ...
              className={`px-4 py-2 text-xs font-bold rounded ${activeTab === 'about' ? 'bg-red-600 text-white' : 'bg-slate-200 text-slate-700'}`}
            >
              Overview
            </button>
            <button
              onClick={() => setActiveTab('portfolio')}
              className={`px-4 py-2 text-xs font-bold rounded ${activeTab === 'portfolio' ? 'bg-red-600 text-white' : 'bg-slate-200 text-slate-700'}`}
            >
              Portfolio ({portfolioImages.length})
            </button>
            <button
              onClick={() => setActiveTab('reviews')}
              className={`px-4 py-2 text-xs font-bold rounded ${activeTab === 'reviews' ? 'bg-red-600 text-white' : 'bg-slate-200 text-slate-700'}`}
            >
              ...
              portfolioImages.length > 0 ? (
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                  {portfolioImages.map((img: string, idx: number) => (
                    <div key={idx} onClick={() => setSelectedImage(img)} className="relative h-40 cursor-pointer overflow-hidden rounded border border-slate-200">
                      <img src={img} alt={`Portfolio ${idx}`} className="h-full w-full object-cover" />
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-slate-500 py-4">No portfolio images available.</p>
              )
            )}
              ...
              reviews.length > 0 ? (
                <div className="space-y-3">
                  {reviews.map((rev: any, idx: number) => (
                    <div key={idx} className="border border-slate-200 bg-white p-4 rounded text-xs">
                      ...
                      <p className="text-slate-600">{rev.comment || 'No comments.'}</p>
                    </div>
                  ))}
                </div>
              ) : (
        ...
        <div className="mt-10 rounded-xl border border-slate-200 bg-white p-6 shadow-md">
            ...
            <div>
              ...
              <div className="relative mt-1">
                <select
                  ...
                  className="w-full appearance-none rounded border border-slate-300 bg-white px-3 py-2 text-xs text-slate-700 focus:border-red-500 focus:outline-none"
                >
