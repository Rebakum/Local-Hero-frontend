...
const STORAGE_KEY = 'localhero_saved_pros';
...
const proId = (p: Professional): string => p.id || p._id || '';
...
const SavedPros: React.FC = () => {
  const { professionals, isLoading, error, refresh } = useProfessionals();
  ...
  const savedPros = useMemo(() => {
    const byId = new Map(professionals.map((p) => [proId(p), p]));
    return savedIds
      .map((id) => byId.get(id))
      .filter((p): p is Professional => Boolean(p));
  }, [professionals, savedIds]);

  const filtered = savedPros.filter(
    (p) =>
      !searchQuery.trim() ||
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between">
          ...
          <div className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/15 backdrop-blur-sm border border-white/20">
            ...
            <span className="text-sm font-medium">{savedPros.length} Saved</span>
          </div>
        </div>
        ...
        >
          <Card padding="lg">
            <div className="flex flex-col items-center text-center py-10">
              ...
              <p className="text-sm font-semibold text-navy-500 dark:text-navy-400">Loading professionals...</p>
            </div>
          </Card>
        </motion.div>
      ) : filtered.length === 0 ? (
          ...
          <Card padding="lg">
            <div className="flex flex-col items-center text-center py-8">
              ...
              <p className="text-sm font-semibold text-navy-500 dark:text-navy-400">
                {savedIds.length === 0 ? 'No saved professionals' : 'No matches found'}
              </p>
            ...
            >
              <Card hover padding="md" className="h-full group relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-4 text-xs text-navy-500 dark:text-navy-400">
                  <span className="inline-flex items-center gap-1"><MapPin className="w-3 h-3" /> {pro.postcodeArea || pro.location || 'N/A'}</span>
                  <span className="inline-flex items-center gap-1"><Calendar className="w-3 h-3" /> {pro.availability || 'Available Today'}</span>
                </div>
