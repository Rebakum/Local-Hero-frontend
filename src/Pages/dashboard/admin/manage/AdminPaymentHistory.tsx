...
export const AdminPaymentHistory: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="text-xs text-gray-400 dark:text-navy-300">{status}</span>;
    }
  };
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase text-gray-500 dark:text-navy-300">Total Revenue</p>
          ...
          <div className="p-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase text-gray-500 dark:text-navy-300">Successful Payments</p>
          ...
          <div className="p-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase text-gray-500 dark:text-navy-300">Pending</p>
          ...
          <div className="p-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase text-gray-500 dark:text-navy-300">Failed / Expired</p>
      ...
      <DataTable<PaymentRecord>
        ...
        columns={[
            ...
            render: (p) => (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-navy-300">{p.booking?.email || "—"}</p>
                </div>
              </div>
            ),
          },
          ...
          },
          {
            ...
            render: (p) =>
              p.stripePaymentIntentId ? (
                <span className="text-xs font-mono text-gray-500 dark:text-navy-300 truncate max-w-[120px] block" title={p.stripePaymentIntentId}>
                  {p.stripePaymentIntentId}
                </span>
              ) : (
                <span className="text-xs text-gray-400 dark:text-navy-300">N/A</span>
              ),
          },
          {
            key: 'date',
            ...
            render: (p) => (
              <span className="text-xs text-gray-500 dark:text-navy-300">
                {p.createdAt
