...
const DEFAULT_TRADE_META: TradeMeta = {
  icon: UserRound,
  pill: 'bg-navy-100 text-navy-800 dark:bg-white/10 dark:text-navy-300',
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2 lg:ml-auto">
          ...
          >
            All trades
            <span className={`flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${tradeFilter === '' ? 'bg-white/20 text-white' : 'bg-navy-100 text-navy-800 dark:bg-white/10 dark:text-navy-300'}`}>
          ...
          {tradeOptions.map((opt) => {
            ...
            return (
              ...
              >
                {opt.label}
                <span className={`flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${active ? 'bg-white/20 text-white' : 'bg-navy-100 text-navy-800 dark:bg-white/10 dark:text-navy-300'}`}>
                  {count}
                </span>
              </button>
            );
              ...
              return (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {p.companyName && (
                      <p className="truncate text-xs text-navy-800 dark:text-navy-300">
                        {p.companyName}
                      </p>
                    )}
                  </div>
          ...
          },
          {
            ...
            render: (p) => (
              <span className="font-bold text-navy-800 dark:text-navy-100">
                £{p.hourlyRate ?? 0}
                <span className="text-[11px] font-medium text-navy-800 dark:text-navy-300">/hr</span>
              </span>
            ),
          },
          {
            ...
            render: (p) =>
              p.rating != null ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-navy-700 dark:text-navy-200">
                  ...
                  <span className="font-normal text-navy-800 dark:text-navy-300">
                    ({p.reviewCount ?? 0})
                  </span>
                </span>
              ) : (
                <span className="text-xs text-navy-800 dark:text-navy-300">—</span>
              ),
          },
        ]}
        actions={(pro) => (
